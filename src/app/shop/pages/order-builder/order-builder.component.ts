...
import { FittingLibraryService } from '../../services/fitting-library.service';
import { OrderStoreService } from '../../services/order-store.service';
import { ConnectionType, FabricationOrder, FabricationPiece, FittingDefinition, HorizontalDirection, TransitionVariant, VerticalDirection } from '../../models/shop.models';
import { GeneratedPreviewComponent } from '../../components/generated-preview/generated-preview.component';
import { WeightCalculationService } from '../../services/weight-calculation.service';
import { FabricationCostEngine } from '../../engines/cost/fabrication-cost.engine';
import { ClientsService, ClientItem } from '../../../features/dashboard/pages/construction/bids/clients.service';

@Component({
  selector: 'app-order-builder',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, GeneratedPreviewComponent],
...
export class OrderBuilderComponent {
  ...
  order: FabricationOrder = this.orderStore.newOrder();
  ...
  constructor(
    ...
    private costEngine: FabricationCostEngine,
    ...
    if (id) {
      this.orderStore.get(id).subscribe(order => {
        if (!order) return;
        this.order = order;
  ...
  get selectedCompany(): ClientItem | undefined {
    return this.companies.find(company => company.name === this.order.customerName || company.id === this.order.customerId);
  }
  ...
  onCustomerChange() {
    ...
    this.order.customerId = selected?.id || '';
    this.order.customerName = selected?.name || this.order.customerName || '';
  ...
  addPiece() {
    ...
    const calculatedPiece = this.weightCalc.applyCalculation(piece);

    this.order.pieces.push(calculatedPiece);
  ...
  recalculateWeights(save = true) {
    this.order.pieces = this.order.pieces.map(piece => {
      return this.weightCalc.applyCalculation(piece);
    });
    if (save) this.saveDraft();
  }


  get shopReportTotals() {
    const totals = {
      sheetMetalFt2: 0,
      tdfLinearFt: 0,
      tdfMaterialFt2: 0,
    ...
    for (const piece of this.order.pieces) {
      const b = piece.manufacturingBreakdown;
      if (!b) continue;
      totals.sheetMetalFt2 += b.sheetMetalFt2 || 0;
      totals.tdfLinearFt += b.connectionMaterial.tdfLinearFt || 0;
      totals.tdfMaterialFt2 += b.connectionMaterial.tdfMaterialFt2 || 0;
      totals.tdcLinearFt += b.connectionMaterial.tdcLinearFt || 0;
      totals.slipPieces += b.connectionMaterial.slipPieces || 0;
      totals.drivePieces += b.connectionMaterial.drivePieces || 0;
      totals.corners += b.accessories.corners || 0;
      totals.screws += b.accessories.screws || 0;
      totals.cleats += b.accessories.cleats || 0;
      totals.tieRods += b.accessories.tieRods || 0;
      totals.gClips += b.accessories.gClips || 0;
      totals.sheetMetalLb += b.weight.sheetMetalLb || 0;
      totals.connectionLb += b.weight.connectionsLb || 0;
      totals.reinforcementLb += b.weight.reinforcementLb || 0;
      totals.totalWeightLb += b.weight.totalLb || 0;
      totals.laborMinutes += b.labor.totalMinutes || 0;
    }

    return {
      ...totals,
      sheetMetalFt2: Math.round(totals.sheetMetalFt2 * 10) / 10,
      tdfLinearFt: Math.round(totals.tdfLinearFt * 10) / 10,
      tdfMaterialFt2: Math.round(totals.tdfMaterialFt2 * 10) / 10,
      tdcLinearFt: Math.round(totals.tdcLinearFt * 10) / 10,
      sheetMetalLb: Math.round(totals.sheetMetalLb * 10) / 10,
      connectionLb: Math.round(totals.connectionLb * 10) / 10,
      reinforcementLb: Math.round(totals.reinforcementLb * 10) / 10,
      totalWeightLb: Math.round(totals.totalWeightLb * 10) / 10,
      laborMinutes: Math.round(totals.laborMinutes * 10) / 10
    };
  }


  get fabricationCostSummary() {
    return this.costEngine.summarize(this.order.pieces);
  }
  ...
  async saveDraft() {
    this.isSaving = true;
    this.saveError = '';
    this.recalculateTotalsOnOrder();
    try {
      await this.orderStore.saveDraft(this.order);
      this.saveMessage = `Draft saved: ${this.order.orderNumber}`;
    } catch (error) {
      console.error('[SHOP] Save draft failed', error);
      this.saveError = 'Could not save draft. Check Firestore permissions or connection.';
    } finally {
      this.isSaving = false;
    }
  }
  ...
  async submitForApproval() {
    await this.changeStatus('Pending Approval');
  }

  async approveOrder() {
    await this.changeStatus('Approved');
  }

  async sendToProduction() {
    await this.changeStatus('In Production');
  }
  ...
  async cancelOrder() {
    if (!confirm('Cancel this fabrication order?')) return;
    await this.changeStatus('Cancelled');
  }

  private async changeStatus(status: FabricationOrder['status']) {
    this.isSaving = true;
    this.saveError = '';
    this.recalculateTotalsOnOrder();
    try {
      this.order.status = status;
      await this.orderStore.save(this.order);
      await this.orderStore.updateStatus(this.order, status);
      this.saveMessage = `Order ${this.order.orderNumber} updated to ${status}.`;
    } catch (error) {
      console.error('[SHOP] Status update failed', error);
      this.saveError = 'Could not update order status. Check Firestore permissions or connection.';
    } finally {
      this.isSaving = false;
    }
  }

  private recalculateTotalsOnOrder() {
    this.order.totalAreaFt2 = this.totalAreaFt2;
    this.order.totalWeightLb = this.totalWeightLb;
    this.order.totalPieces = this.order.pieces.reduce((sum, p) => sum + Number(p.quantity || 1), 0);
  }

  printApproval() {
    document.body.classList.add('shop-print-mode');

    const cleanup = () => document.body.classList.remove('shop-print-mode');
  ...
  clearOrder() {
    if (!confirm('Clear current order? Unsaved changes will be lost.')) return;
    this.order = this.orderStore.newOrder();
    this.selectedPieceId = '';
    this.saveMessage = '';
    this.saveError = '';
  }
